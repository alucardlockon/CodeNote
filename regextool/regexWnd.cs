...
{
    public partial class regexWnd : Form
    {
        public regexWnd()
        {
            InitializeComponent();
        ...
        private void btnRegYZ_Click(object sender, EventArgs e)
        {
            Regex reg = new Regex(regValue.Text.Trim());
            MatchCollection result=reg.Matches(regText.Text.Trim().ToString());
            if (result.Count>0)
            {
                regResultLabel.Text = "共" + result.Count + "处匹配";
                string resultStr = "";
                foreach (var res in result)
	            {
		            resultStr += res.ToString()+ " ";
	            }
                regResult.Text = resultStr;
            }
            else
            {
                regResultLabel.Text = "共0处匹配";
                regResult.Text = "未匹配";
            }
        }

        private void btnJsonYZ_Click(object sender, EventArgs e)
        {
            int errIndex;
            bool result =IsJson(regText.Text, out errIndex);
            if (result)
            {
                regResult.Text = "Json合法";
            }
            else
            {
                regResult.Text = "错误位置:"+errIndex;
            }
